...
completeData = """iyr:2013 hcl:#ceb3a1
    hgt:151cm eyr:2030
    byr:1943 ecl:grn
...
#Valid
dataValid = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
    hcl:#623a2f
...
def containsValidByr(password):
    # byr (Birth Year) - four digits; at least 1920 and at most 2002.
    byr = re.search(r"byr:(\d\d\d\d)", password)
    if not byr or int(byr.group(1)) < 1920 or 2002 < int(byr.group(1)):
        return False
    else:
        return True

def containsValidIyr(password):
    # iyr (Issue Year) - four digits; at least 2010 and at most 2020.
    iyr = re.search(r"iyr:(\d\d\d\d)", password)
    if not iyr or int(iyr.group(1)) < 2010 or 2020 < int(iyr.group(1)):
        return False
    else:
        return True

def containsValidEyr(password):
    # eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
    eyr = re.search(r"eyr:(\d\d\d\d)", password)
    if not eyr or int(eyr.group(1)) < 2020 or 2030 < int(eyr.group(1)):
        return False
    else:
        return True

def containsValidHtg(password):
    # hgt (Height) - a number followed by either cm or in:
    hgt = re.search(r"hgt:(\d{2,3})(in|cm)", password)
    # If cm, the number must be at least 150 and at most 193.
    if not hgt or hgt.group(2) == "cm" and (int(hgt.group(1)) < 150 or 193 < int(hgt.group(1))):
        return False
    # If in, the number must be at least 59 and at most 76.
    elif not hgt or hgt.group(2) == "in" and (int(hgt.group(1)) < 59 or 76 < int(hgt.group(1))):
        return False
    else:
        return True
        
def containsValidHcl(password):
    # hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.
    hcl = re.search(r"hcl:#([0-9a-f]{6})", password)
    if not hcl:
        return False
    else:
        return True
        
def containsValidEcl(password):
    # ecl (Eye Color) - exactly one of: amb blu brn gry grn hzl oth.
    ecl = re.search(r"ecl:(amb|blu|brn|gry|grn|hzl|oth)", password)
    if not ecl:
        return False
    else:
        return True
        
def containsValidPid(password):
    # pid (Passport ID) - a nine-digit number, including leading zeroes.
    pid = re.search(r"pid:\d{9}( |\n|$)", password)
    if not pid:
        return False
    else:
        return True

def containsAllFields(password):
    if containsValidByr(password) and containsValidEcl(password) and containsValidEyr(password) and containsValidHcl(password) and containsValidHtg(password) and containsValidIyr(password) and containsValidPid(password):
        return True
    else:
        return False
...
def doIt(data):
    ...
    for password in passwords:
        if containsAllFields(password):
            validPasswords += 1
    return validPasswords

print(doIt(dataValid))
print(doIt(dataInvalid))
print(doIt(completeData))