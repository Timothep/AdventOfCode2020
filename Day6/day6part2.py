...
def getUniqueCount(line):
    uniqueCharacters = ''.join(set(line))
    return uniqueCharacters.__len__()

# input -> [group, group, group] with redundancies
def squashMultilines(multilines):
    multilines = multilines.replace(' ', '')
    groups = multilines.split("\n\n")    
    outputList = []
    for group in groups:
        outputList.append(group.split('\n'))
    print(outputList)
    return outputList

def intersectionOfStrings(string1, string2):
    result = ''
    for char in string1:
...
def countLettersInAllElements(elementsList):
    #go through all the letters and check for each if it is in all others
    intersection = elementsList[0]
    for element in elementsList:
        intersection = intersectionOfStrings(intersection, element)
    return intersection.__len__()

cleanedUpData = squashMultilines(completeData)
count = 0
for group in cleanedUpData:
    count += countLettersInAllElements(group)
