...
dictionary = parseWholeData(completeData)
#print(dictionary)
...
reversed = reverseDictionary(dictionary)
#print(reversed)
...
def getChildren(node, listSofar):
    # if there is no child anymore (key not found)
    if node not in reversed:
        return []
    else:
        ...
        for subnode in reversed[node]:
            if not listSofar.__contains__(subnode):
                listSofar.append(subnode)
                newFoundChildren.append(subnode)

        for newFoundChild in newFoundChildren:
            getChildren(newFoundChild, listSofar)

#start with shinygold
outputList = []
getChildren("shiny gold", outputList)
print(outputList)
print(outputList.__len__())
