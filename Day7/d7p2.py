...
completeData = """clear purple bags contain 5 faded indigo bags, 3 muted purple bags.
...
def extractParts(line):
    bagNameRegexObject = re.search(r"(.*) bags contain ", line)
    bagName = bagNameRegexObject.group(1).strip() if bagNameRegexObject else ''

    otherBagsStringsRegexObject = re.search(r".* contain (.*)\.", line)
    otherBagsStrings = otherBagsStringsRegexObject.group(1).split(',') if otherBagsStringsRegexObject else ''
    
    value = []
    for otherBagString in otherBagsStrings:
        regex = re.search(r"(\d) (.*) bags?", otherBagString)
        if not regex: continue
        value.append([regex.group(2).strip(), regex.group(1)])
    return [bagName, value]

def parseWholeData(data):
    dictionary = {}
    cleanData = data.split('\n')

    for line in cleanData:
        kvp = extractParts(line)
        dictionary[kvp[0]] = kvp[1]
    return dictionary

#reverse Dictionary
# go through all keys, go through all values
# if value !exists as key in new dictionary, add it with old key as value
# if value exists as key in new dictionary, add old key as additional value
...
def reverseDictionary(inputDictionary):
    ...
    for key in inputDictionary:
        #traverse the values
        for value in inputDictionary[key]:
            #if the key doesn't already exist
            if not value[0] in reversedDictionary:
                reversedDictionary[value[0]] = [key]
            else:
                valueToUpdate = reversedDictionary[value[0]]
                valueToUpdate.append(key)
                reversedDictionary[value[0]] = valueToUpdate
...
def getChildren(node, factor, listSofar):
    # if there is no child anymore (key not found)
    if node not in bagsDictionary:
        return []
    else:
        # gather all the children
        newFoundChildren = []
        for subnode in bagsDictionary[node]:
            #if not listSofar.__contains__(subnode):
            newNode = subnode
            newNode.append(factor)
            listSofar.append(newNode)
            newFoundChildren.append(newNode)

        for newFoundChild in newFoundChildren:
            getChildren(newFoundChild[0], newFoundChild[1], listSofar)

bagsDictionary = parseWholeData(completeData)
reversed = reverseDictionary(bagsDictionary)
input = "shiny gold"

outputList = []
getChildren(input, 1, outputList)
print(outputList)

total = 0

for bag in outputList:
    numberOfBags = int(bag[1])
    length = bag.__len__()
    sub = 1
    for num, value in enumerate(bag, start=2):
        if num >= length:
            break
        sub = sub * int(bag[num])
    total += sub
